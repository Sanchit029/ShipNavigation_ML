...
from uielements import draw_quit_button
...
def check_quit_event(quit_button_rect):
    """Check for quit events and handle Force Quit."""
...
def a_star(start, end, is_first_box_green, is_second_box_green):
    ...
    explored_nodes = []
    quit_button_rect = draw_quit_button(screen)  # Draw the Force Quit button
    while not open_set.empty():
        _, current = open_set.get()
        check_quit_event(quit_button_rect)
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            quit_button = draw_quit_button(screen)
    ...
    # After drawing other UI elements
